...
class Fork(threading.Thread):
    ...
    def run(self):
        log.LogMsg(Logger.INFO, 'Fork started running')
        ...
        while True:
            ...
            if data is _shutdown:
                log.LogMsg(Logger.INFO, 'Fork received shutdown')
                self.q_in.task_done()
                break
            else:
                log.LogMsg(Logger.INFO, 'Fork received ' + str(data) + ' to process')
                for q in self.q_out:
                    q.put(data*2)
                self.q_in.task_done()
        ...
        log.LogMsg(Logger.INFO, 'Fork finished running')